...
export class UsersService {
  ...
  GetAssignedRolePermissionsByUserId_Promise(userId: number) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.GetAssignedRolePermissionsByUserId(userId, 0).subscribe(result => {
        resolve(this.userPermission);
      });
    });
  }

  GetAssignedRolePermissionsByUserId(userId: number, appId?: any): Observable<any> {
    const moduleKey = 'hmis';
    const reqUrl = `${environment.baseUrl}/RolePermission/GetAssignedRolePermissionsByModule/${userId}/${moduleKey}`;
  ...
  GetCompanyList(reqParam){
    const reqUrl = `${environment.baseUrlHis}/Patient/GetCompanyListByType`;
  ...
  getAllRevenueDashboardData(){
    const reqUrl = `${environment.baseUrlHis}/PatientBilling/GetAllRevenueDashboardData`;
