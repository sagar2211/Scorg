import { Doctor } from '@qms/qlist-lib';
import { Patient } from './../public/models/patient.model';
...
@Component({
  selector: 'app-emr-side-menu',
  templateUrl: './emr-side-menu.component.html',
  styleUrls: ['./emr-side-menu.component.scss']
})
export class EmrSideMenuComponent implements OnInit, OnDestroy {
  @Input() showMenuType;
  ...
  onMenuClick(parentUrl): void {
    const indx = this.favouriteMenusList.findIndex(f => f.parentMenu === parentUrl.linkKey && f.masterModule === 'emr');
  ...
  updateSideMenuList() {
    this.sideBarArray = this.sideMenuService.sideMenuListEmr;
  }
