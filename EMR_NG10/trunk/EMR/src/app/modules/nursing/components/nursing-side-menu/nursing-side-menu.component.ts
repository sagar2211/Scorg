...
@Component({
  selector: 'app-nursing-side-menu',
  templateUrl: './nursing-side-menu.component.html',
  styleUrls: ['./nursing-side-menu.component.scss']
})
export class NursingSideMenuComponent implements OnInit {
  @Input() showMenuType;
  sideBarArray = [];
  userInfo: any;
  settingKey = '';
  ...
  saveFavourite(parentItem, childItem, $event): void {
    ...
    const indx = this.favouriteMenusList.findIndex(f => f.parentMenu === parentItem.linkKey && f.masterModule === 'nursingApp');
    if (indx !== -1) {
      this.favouriteMenusList[indx].favouriteRouteUrl = 'nursingApp/nursing/' + childItem.linkKey;
    } else {
      this.favouriteMenusList.push({
        masterModule: 'nursingApp',
        parentMenu: parentItem.linkKey,
        favouriteRouteUrl: 'nursingApp/nursing/' + childItem.linkKey
      });
    }
  ...
  onMenuClick(parentUrl): void {
    const indx = this.favouriteMenusList.findIndex(f => f.parentMenu === parentUrl.linkKey && f.masterModule === 'nursingApp');
  ...
  updateSideMenuList() {
    this.sideBarArray = this.sideMenuService.sideMenuListNursing;
  }
