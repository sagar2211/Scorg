...
export class BedService {
  ...
  getLevelCofiguration(): Observable<any> {
    const reqUrl = `${environment.HIS_WEB_APP_URL}/api/VisualBedBrowser/getLevelConfigList`;
  ...
  getBedChargesList(): Observable<any> {
    const reqUrl = `${environment.HIS_WEB_APP_URL}/api/VisualBedBrowser/GetBedChargeList`;
    return this.http.get(reqUrl).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.data.length > 0) {
        return res.data;
      } else {
  ...
  getBedCategoryList(): Observable<any> {
    const reqUrl = `${environment.HIS_WEB_APP_URL}/api/VisualBedBrowser/GetBedCategoryList`;
    return this.http.get(reqUrl).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.data.length > 0) {
        return res.data;
      } else {
  ...
  getBedTypeList(): Observable<any> {
    const reqUrl = `${environment.HIS_WEB_APP_URL}/api/VisualBedBrowser/GetBedTypeList`;
    return this.http.get(reqUrl).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.data.length > 0) {
        return res.data;
      } else {
  ...
  getBedClassList(): Observable<any> {
    const reqUrl = `${environment.HIS_WEB_APP_URL}/api/VisualBedBrowser/GetBedClassList`;
    return this.http.get(reqUrl).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.data.length > 0) {
        return res.data;
      } else {
  ...
  getBedOccupiedList(param): Observable<any> {
    ...
    // ]
    // );
    const reqUrl = `${environment.HIS_WEB_APP_URL}/api/VisualBedBrowser/GetOccupiedBedDetail`;
    return this.http.post(reqUrl, param).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.data.length > 0) {
        return res.data;
      } else {
  ...
  getPatientBedData(id): Observable<any> {
    const reqUrl = `${environment.HIS_WEB_APP_URL}/api/VisualBedBrowser/GetPatientBedData&patientBedAssignId=${id}`;
    return this.http.get(reqUrl).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.data.length > 0) {
        return res.data;
      } else {
