...
export class DischargeSideMenuComponent implements OnInit {
  ...
  saveFavourite(parentItem, childItem, $event): void {
    ...
    if (indx !== -1) {
      ...
      this.favouriteMenusList.push({
        masterModule: 'dischargeApp',
        parentMenu: parentItem.linkKey,
        favouriteRouteUrl: 'dischargeApp/discharge/' + childItem.linkKey
      });
    }
  ...
  onMenuClick(parentUrl): void {
    const indx = this.favouriteMenusList.findIndex(f => f.parentMenu === parentUrl.linkKey && f.masterModule === 'dischargeApp');
    ...
    this.commonService.getQueueSettings(this.settingKey, this.userInfo.user_id).subscribe(res => {
      if (res) {
        ...
        this.favouriteMenusList.forEach((fav: any) => {
          const i = this.sideBarArray.findIndex(r => r.linkKey === fav.parentMenu && fav.masterModule === 'dischargeApp');
          if (i !== -1) {
  ...
  updateSideMenuList() {
    this.sideBarArray = this.sideMenuService.sideMenuListDischarge;
  }
