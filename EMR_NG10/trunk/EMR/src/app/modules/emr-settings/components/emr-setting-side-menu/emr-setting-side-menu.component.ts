...
@Component({
  selector: 'app-emr-setting-side-menu',
  templateUrl: './emr-setting-side-menu.component.html',
  styleUrls: ['./emr-setting-side-menu.component.scss']
})
export class EmrSettingSideMenuComponent implements OnInit {
  @Input() showMenuType;
  sideBarArray = [];
  userInfo: any;
  settingKey = '';
  ...
  saveFavourite(parentItem, childItem, $event): void {
    ...
    if (indx !== -1) {
      this.favouriteMenusList[indx].favouriteRouteUrl = 'emrSettingsApp/settings/' + childItem.linkKey;
    } else {
      this.favouriteMenusList.push({
        masterModule: 'emrSettingsApp',
        parentMenu: parentItem.linkKey,
        favouriteRouteUrl: 'emrSettingsApp/settings/' + childItem.linkKey
      });
    }
  ...
  onMenuClick(parentUrl): void {
    const indx = this.favouriteMenusList.findIndex(f => f.parentMenu === parentUrl.linkKey && f.masterModule === 'emrSettingsApp');
    ...
    this.commonService.getQueueSettings(this.settingKey, this.userInfo.user_id).subscribe(res => {
      if (res) {
        ...
        this.favouriteMenusList.forEach((fav: any) => {
          const i = this.sideBarArray.findIndex(r => r.linkKey === fav.parentMenu && fav.masterModule === 'emrSettingsApp');
          if (i !== -1) {
  ...
  updateSideMenuList() {
    this.sideBarArray = this.sideMenuService.sideMenuListEmrSetting;
  }
