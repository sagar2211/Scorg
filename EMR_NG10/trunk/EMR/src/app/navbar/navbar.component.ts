...
export class NavbarComponent implements OnInit, OnDestroy {
  ...
  nursingStation: { nursingStationId: any; nursingStationName: any; };
  selectedNursingStation: any;
  userId: any;
  stationArray: any;
  stationArrayFiltered: any[];
  ...
  ngOnInit() {
    ...
    this.loginFor = JSON.parse(localStorage.getItem('loginFor'));
    // Active User Name
    this.loginUserName = this.authService.getUserInfoFromLocalStorage()?.user_name;
    this.nursingStationName = this.authService.getUserInfoFromLocalStorage()?.nursingStationName;
  }
  ...
  subsCriptionEvent() {
    ...
    this.commonService.$subAddRemoveActivePatientFromList.pipe(takeUntil(this.$navSelection)).subscribe((obj: any) => {
      ...
      } else if (obj.data && obj.sourc === 'navbar') {
        const patListData = _.clone(this.commonService.getActivePatientList());
        this.updatePatientIndex(obj.data, patListData);
      } else {
        const patListData = _.clone(this.commonService.getActivePatientList());
        this.updatePatientIndex(obj, patListData);
      }
    });
  ...
  setModuleTitleAndNav() {
    ...
    if (this.moduleName && this.moduleName === 'emr') {
      this.selectedModule.navLink = '/emr';
      this.selectedModule.displayValue = 'DOCTOR DESK';
    } else if (this.moduleName && this.moduleName === 'nursing') {
      this.selectedModule.navLink = '/nursingApp';
      this.selectedModule.displayValue = 'NURSING';
    } else if (this.moduleName && this.moduleName === 'discharge') {
      this.selectedModule.navLink = '/dischargeApp';
      this.selectedModule.displayValue = 'DISCHARGE';
    } else if (this.moduleName && this.moduleName === 'emr settings') {
      this.selectedModule.navLink = '/emrSettingsApp';
      this.selectedModule.displayValue = 'EMR SETTINGS';
    }
  ...
  getdefultLandingPageOnUserLogin(app: any) {
    if (app.app_key === 'emr' || _.toLower(app.app_key) === 'emr-settings' || (app.app_key === 'nursing' || app.app_key === 'NURSING') || (app.app_key === 'discharge' || app.app_key === 'DISCHARGE')) {
      const key = this.authService.getActiveAppKey();
      const appKey = key === 'EMR' ? 'emr' :
        (key === 'NURSING' ? 'nursing' :
          (_.toLower(key) === 'emr-settings' ? 'emr-settings' : 'discharge'));
      if (!(_.lowerCase(appKey) === app.app_key)) {
  ...
  goToPatientPage(patient, indx) {
    ...
    this.showHideMainMenuStatus = false;
    this.showHidePatientMenu = false;
    const appKey = this.authService.getActiveAppKey() === 'EMR' ? 'emr' :
      (this.authService.getActiveAppKey() === 'NURSING' ? 'nursing' :
        (_.toLower(this.authService.getActiveAppKey()) === 'emr-settings' ? 'emr-settings' : 'discharge'));
    let navigationUrl = null;
    if (appKey === 'emr') {
      navigationUrl = '/emr/';
    } else if (appKey === 'nursing') {
      navigationUrl = '/nursingApp/nursing/';
    } else if (appKey === 'discharge') {
      navigationUrl = '/dischargeApp/discharge/';
    }
    patient.lastActiveUrl = patient.lastActiveUrl || (navigationUrl + '/' + patient.patientData.id);
    ...
    this.router.navigate([patient.lastActiveUrl]);
  ...
  selectedPatient(patient) {
    ...
    if (this.commonService.getActivePatientList(patient).length <= 5) {
      ...
      if (this.moduleName && this.moduleName === 'emr') {
        ...
        this.router.navigate(['/nursingApp/nursing/patient/dashboard/', patient.patientData.id]);
      } else if (this.moduleName && this.moduleName === 'discharge') {
