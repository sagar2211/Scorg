...
export class RadiologyTestService {
  ...
  getRadiologyTestData(): Observable<any> {
    ...
    const radioTestData = this.consultationService.getConsultationFormDataByKey(patientId, 'radioInvestigationInputs');
    if (radioTestData && radioTestData.length) {
      ...
      return of(this.consultationService.setConsultationFormData(patientId, 'radioInvestigationInputs', defaultObj));
    }
  }

  checkIfDataExist(): boolean {
    const indx = _.findIndex(this.radiologyTestInputs, o => o.formId == this.publicService.activeFormId);
  ...
  setRadiologyTestData(formData, mode, indx?): void {
    ...
    if (mode == 'add' || mode == 'update') {
      radiologyTestData = this.consultationService.setConsultationFormData(patientId, 'radioInvestigationInputs', formData, mode);
      if (radiologyTestData.length > 1) {
        _.remove(radiologyTestData, obj => obj.label == '');
      }
    } else {
      // delete mode
      radiologyTestData = this.consultationService.getConsultationFormDataByKey(patientId, 'radioInvestigationInputs');
  ...
  //   const patientId = this.consultationService.getPatientObj('patientId');
  //   let radiologyTestData;
  //   const formIndx = _.findIndex(this.radiologyTestInputs, (o) => o.formId == this.publicService.activeFormId);
  //   switch (mode) {
