  ...
  transform(items: any, filter?: any, categoryFilter?: boolean, savedUnsavedOrdersFlag?, statusFilter?): any {
    if (items) {
      if (savedUnsavedOrdersFlag !== undefined && savedUnsavedOrdersFlag !== false) {
        ...
        _.forEach(items, (o) => {
          let saved = true;
          saved = !(!o.id || o.id === undefined || o.id === '');
      ...
      if (filter && filter.approveOrderDate) {
        items = _.filter(items, (o1) => o1.approved_by && o1.approved_by.approved_date === filter.approveOrderDate);
      }
      if (filter && filter.orderDate) {
        items = _.filter(items, (o1) => o1.order_date === filter.orderDate);
      }
      if (filter &&  _.isArray(filter.orderStatusList) && filter.orderStatusList.length) {
        items = _.filter(items, (o1) => _.some(filter.orderStatusList, (o2) => o2.orderKey === o1.categoryKey));
      }
