...
@Component({
  selector: 'app-user-to-ward',
  templateUrl: './user-to-ward.component.html',
  styleUrls: ['./user-to-ward.component.scss']
})
export class UserToWardComponent implements OnInit, OnDestroy {
  ...
  mappingRmoDept(data?) {
    ...
    modalInstance.componentInstance.mappingToObj = { mappingToKey: 'ward', label: 'Ward', placeholder: 'Select Ward'};
    modalInstance.componentInstance.saveObject.subscribe((e: any) => {
      this.mappingNurseToward(e);
    });
  ...
  getListData() {
    ...
    this.masterMappingService.getNursingWordMappingList(param).subscribe(res => {
      if (res.total_records > 0) {
        this.nursWardMappingList = res.data;
        this.page.totalElements = res.total_records;
      } else {
        this.nursWardMappingList = [];
  ...
  deleteById(rowData) {
    this.masterMappingService.deleteNursingWordMappingById(rowData.id).subscribe(res => {
      if (res) {
        this.notifyAlertMessage({
  ...
  editData(row) {
    const obj = {
      ...
      mapTo: { id: row.ward_id, name: row.ward_name },
