import { QueueSlots, QSlotInfo } from '@qms/qlist-lib';
...
export class CommonService {
  ...
  openSuggesstionPanelOnFixedComponentSearchAlways: Subject<any> = new Subject();
  ...
  openSuggesstionPanelWhenTabModeOn: Subject<any> = new Subject();
  ...
  isPatientOpenFromNursingRoute = false;
  patientNursingRoute = null;
  constructor(
  ...
  openSuggesstionPanelOnFixedComponentSearchCallFunction(val) {
    this.openSuggesstionPanelOnFixedComponentSearchAlways.next(val)
  }
  ...
  GetAssignedRolePermissionsByUserId(userId: number): Observable<any> {
    ...
    const appId = this.authService.getAppIdByAppKey(appKey);
  ...
  convertAppointmentDataToFlatList(list: Array<QSlotInfo>, timeFormateKey, queueType?, queuename?, queueTypeId?, queueValueId?): Array<QueueSlots> {
    const tempList = [];
    list.forEach(r => {
  ...
  getQueueSettingsForMultiple(param, template?): Observable<any> {
    const reqUrl = `${environment.baseUrlAppointment}/QMSSettings/getMultipleQmsSettings`;
  ...
  getPrintContent(param): Observable<any> {
    const reqUrl = `${environment.HIS_WEB_APP_URL}/ShowReport/GetOpdPrint`;
  ...
  getAllRecentPatientList(userId): Observable<any> {
    // const reqUrl = `${environment.dashboardBaseURL}/emr/GetUserRoleTypeList`;
    // return this.http.get(reqUrl).pipe(map((res: any) => {
    //   if (res.status_message === 'Success') {
    //     const temp = [];
