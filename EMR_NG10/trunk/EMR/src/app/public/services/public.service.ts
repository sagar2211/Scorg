...
export class PublicService {
  ...
  savePatientBedTransfer(param): Observable<any> {
    const reqUrl = `${environment.HIS_WEB_APP_URL}/api/VisualBedBrowser/SavePatientBedTransfer`;
    return this.http.post(reqUrl, param).pipe(
      map((res: any) => {
        return res;
      }));
  }
  GetPatientBedTransferDetail(param): Observable<any> {
    const reqUrl = `${environment.dashboardBaseURL}/PatientTransfer/GetPatientBedTransferDetail`;
    return this.http.post(reqUrl, param).pipe(
      map((res: any) => {
       if (res.status_message === 'Success') {
          return res.data;
  ...
  GethandoverPatient(param): Observable<any> {
    const reqUrl = `${environment.dashboardBaseURL}/NursingHandover/GetPatientTransferList`;
    return this.http.post(reqUrl, param).pipe(
      map((res: any) => {
       if (res.status_message === 'Success' && res.data.length !== 0) {
          return res.data;
  ...
  handOverpatientNursing(param): Observable<any> {
    const reqUrl = `${environment.dashboardBaseURL}/NursingHandover/PatientNursingHandover`;
    return this.http.post(reqUrl, param).pipe(
      map((res: any) => {
          return res;
      }));
