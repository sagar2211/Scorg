...
export class PatientService {
  ...
  UpdateDischargeTentativeDays(param): Observable<any> {
    const reqUrl = `${environment.HIS_WEB_APP_URL}/PatientEncounter/UpdateDischargeTentativeDays`;
    return this.http.post(reqUrl, param).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.data) {
        return res.data;
      } else {
  ...
  getPatientDetail(param){
    const reqUrl = environment.HIS_WEB_APP_URL + '/api/Patient/GetPatientsDetailById?uhId='+param.uhId;
    return this.http.get(reqUrl).pipe(
      map((res: any) => {
  ...
  saveDeceasedPatient(param){
    const reqUrl = `${environment.dashboardBaseURL}/Discharge/PatientDeceased`;
    return this.http.post(reqUrl, param).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.data) {
        return res.data;
      } else {
