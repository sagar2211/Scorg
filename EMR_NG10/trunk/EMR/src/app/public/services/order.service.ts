...
export class OrderService {
  ...
  getOrderList(reqParam) {
    const reqUrl = environment.dashboardBaseURL + '/Order/GetOrderDetails';
  ...
  checkOrderStatus(userData) {
    let status = 'INITIATED';
    let approvedBy = null;
    if (this.orderStatusDefaultValue) {
      // case 1: if login user type doctor and same doctor is selected in orderby dropdown
      status = 'APPROVED';
      approvedBy = {
    ...
    } else if ((userData.role_type === _.toUpper(Constants.userRoleType.doctor.key))
      && (this.selectedDoctorForOrder && userData.user_id === this.selectedDoctorForOrder.user_id)) {
      // case 1: if login user type doctor and same doctor is selected in orderby dropdown
      status = 'APPROVED';
      approvedBy = {
        approved_date: new Date(),
      ...
      && (this.selectedDoctorForOrder && userData.user_id !== this.selectedDoctorForOrder.user_id)) {
      // case 2: if login user type doctor and same doctor is not selected in orderby dropdown
      status = 'INITIATED';
      approvedBy = null
    } else if ((userData.role_type === _.toUpper(Constants.userRoleType.doctor.key))) {
      // case 2: if login user type not doctor
      status = 'INITIATED';
      approvedBy = null
    }
    return { status: status, approvedBy: approvedBy }
  }

  saveKeyWiseData(param): Observable<any> {
    const reqParam = _.cloneDeep(param);
    if (param.key === Constants.orderStatusDefaultKey) {
      this.orderStatusDefaultValue = param.value
    }
  ...
  getKeyWiseData(param): Observable<any> {
    ...
    return this.http.post(reqUrl, param).pipe(map((res: any) => {
      if (res.status_code === 200 && res.status_message === 'Success' && res.data.length) {
        if (param.key[0] === Constants.orderStatusDefaultKey && res.data[0].value === 'APPROVED') {
          this.orderStatusDefaultValue = 'APPROVED'
        } else {
          this.orderStatusDefaultValue = null;
        }
        return res.data;
      } else {
        return [];
