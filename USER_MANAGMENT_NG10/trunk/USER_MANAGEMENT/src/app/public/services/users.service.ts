...
import { ModuleMasterList } from '../models/module-master';
...
export class UsersService {
  ...
  getModuleList(): Observable<any> {
    const reqUrl = environment.baseUrl + '/Application/GetModuleList';
    const moduleModel = new ModuleMasterList();
    const modulelist: Array<ModuleMasterList> = [];
    if (this.moduleListMaster.length) {
      return of(this.moduleListMaster[0]);
    } else {
      return this.http.get(reqUrl).pipe(
        map((res: any) => {
          res.data.forEach((obj, index) => {
            if (moduleModel.isObjectValid(obj)) {
              moduleModel.generateObject(obj);
              modulelist.push(Object.assign({}, moduleModel));
            }
          });
          res.data = modulelist;
          this.moduleListMaster.push(res);
          return res;
        })
      );
    }
  ...
  getRoleById(id): Observable<any> {
    const reqUrl = environment.baseUrl + '/RoleMaster/getRoleById?id=' + id;
    const roleTypeModel = new RoleType();
    const roleTypeList: Array<RoleType> = [];
    // if (this.roleTypeListMaster.length) {
    //   return of(this.roleTypeListMaster[0]);
  ...
  getUserDataById(userId, loginUser?): Observable<any> {
    const appId = this.authService.getAppIdByAppKey('user_management');
  ...
  isExistRoleName(applicationId, roleName): Observable<any> {
    const reqUrl = environment.baseUrl + '/UserIdentity/checkDuplicateRoleName';
    const paramVaue = { application_id: applicationId, role_name: roleName };
  ...
  getPrimaryPermissionByRoleId(id): Observable<any> {
    ...
    const reqUrl = environment.baseUrl + '/RolePermission/GetLandingPageListByRoleId?role_id=' + id;
    return this.http.get(reqUrl).pipe(map((res: any) => {
      if (res.status_code === 200) {
        this.allAppPermissionList.push({ roleId: id, data: [...res.data] });
        return res.data;
      } else {
  ...
  getPrimaryPermissionByAppId(id): Observable<any> {
    if (this.allAppPermissionListViaAppId.length > 0) {
      const check = _.find(this.allAppPermissionListViaAppId, d => {
        return d.appId === id;
      });
      if (check) {
        return of(check.data);
    ...
    const reqUrl = environment.baseUrl + '/RolePermission/GetLandingPageListByAppId?appId=' + id;
    return this.http.get(reqUrl).pipe(
      map((res: any) => {
        if (res.status_code === 200) {
          this.allAppPermissionListViaAppId.push({ appId: id, data: res.data });
          return res.data;
        } else {
          this.allAppPermissionListViaAppId.push({ appId: id, data: [] });
          return [];
        }
      })
    );
  ...
  changeUserAppLandingPage(param): Observable<any> {
    const reqUrl = environment.baseUrl + '/RolePermission/ChangeUserAppLandingPage';
    return this.http.post(reqUrl, param).pipe(
      map((res: any) => {
        return res;
      })
  ...
  getUserServiceMappingList(param){
    const reqUrl = `${environment.baseUrl}/UserServiceCenterMap/GetUserServiceCenterMappingList`;
    return this.http.post(reqUrl,param).pipe(
      map((res: any) => {
  ...
  getUserServiceCenterMappingList(userId):Observable<any>{
    const reqUrl = `${environment.baseUrl}/UserServiceCenterMap/GetUserServiceCenterMappingByUserId?userId=`+ userId;
    return this.http.get(reqUrl).pipe(
      map((res: any) => {
  ...
  saveUserStoreMapping(params): Observable<any> {
    const reqUrl = environment.baseUrl + '/UserServiceCenterMap/SaveUserServiceCenterMapping';
    return this.http.post(reqUrl, params).pipe(map((res: any) => {
      return !!(res.status_code === 200 && res.status_message === 'Success' && res.data);
    }));
  }

  getUserStoreMappingByUserId(userId): Observable<any> {
    const reqUrl = environment.baseUrl + '/UserServiceCenterMap/GetUserServiceCenterMappingByUserId?userId='+userId;
    return this.http.get(reqUrl).pipe(map((res: any) => {
  ...
  getCasePaperFollowupByDesignationId(): Observable<any> {
    const reqUrl = environment.baseUrl + '/UserCasePaperValidity/GetCasePaperFollowupByDesignationId';
    return this.http.get(reqUrl).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.status_code === 200) {
        return res.data;
      } else {
  ...
  saveCasePaperValidityAndFollowup(params): Observable<any> {
    const reqUrl = environment.baseUrl + '/UserCasePaperValidity/SaveCasePaperValidityAndFollowup';
    return this.http.post(reqUrl, params).pipe(map((res: any) => {
      if (res.status_message === 'Success' && res.status_code === 200) {
        return res;
      } else {
