...
export class AuthIntercept implements HttpInterceptor {
  ...
  unAuthorizedReq(): boolean {
    const unAuthorizedReq = ['login', 'display/displayList', 'forgotpassword', 'resetforgotpassword/:id', 'external/book-appointment', 'LoginThroughSSO']; // -- give state url
    ...
    if (unAuthorizedReq.some(v => this.router.url.includes(v))) {
      if ((this.router.url === '/login' && this.authService.isLoggedIn()) ||
        (this.router.url.includes('loginThroughSSO') && this.authService.isLoggedIn())
      ) {
        isUnAuthorize = false;
      } else {
        isUnAuthorize = true;
