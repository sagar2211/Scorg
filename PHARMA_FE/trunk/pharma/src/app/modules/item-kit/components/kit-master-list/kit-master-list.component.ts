...
import { KitMasterAddUpdateComponent } from '../kit-master-add-update/kit-master-add-update.component';

@Component({
  selector: 'app-kit-master-list',
  templateUrl: './kit-master-list.component.html',
  styleUrls: ['./kit-master-list.component.scss']
})
export class KitMasterListComponent implements OnInit {
  ...
  defaultObject() {
    ...
    this.sortUserList = { sort_order: 'desc', sort_column: 'kitId' };
  ...
  getItemList(): void {
    ...
    this.kitMastersService.getItemKitMasterList(reqParams).subscribe(res => {
  ...
  editItemMaster(row): void {
    this.kitMastersService.getKitDataByID(row.kitId).subscribe(response => {
  ...
  openPopup(kit) {
    ...
    const modalInstance = this.modalService.open(KitMasterAddUpdateComponent,
      {
    ...
    modalInstance.componentInstance.kitData = kitDetail;
  }
  ...
  deleteItem(row) {
    ...
    modalInstance.result.then((result) => {
      if (result === 'Ok') {
        this.kitMastersService.deleteKitDataById(row.kitId).subscribe(response => {
          if (response.status_code === 200 && response.status_message === 'Success') {
            this.getItemList();
            this.alertMsg = {
  ...
  showActivePopup() {
    this.commonService.$addPopupEvent.pipe(takeUntil(this.$destroy)).subscribe(popup => {
      if (popup && popup.redirectFromPage === '/inventory/masters/kit/itemKitList') {
