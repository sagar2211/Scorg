...
export class PurchaseInvoiceListComponent implements OnInit {
  ...
  onEdit(row): void {
    this.route.navigate(['/inventory/transactions/purchaseInvoice/purchaseInvoiceAddUpdate/', row.invoiceId]);
  }
  ...
  rejectInvoice(reason, row): void {
    ...
    this.transactionService.rejectInvoice(reqParams).subscribe(res => {
      if (res) {
        ...
        this.route.navigate(['/inventory/transactions/purchaseInvoice/purchaseInvoiceList']);
      } else {
        this.notifyAlertMessage({
