...
export class PurchaseReturnListComponent implements OnInit, OnDestroy {
  ...
  editData(val) {
    this.router.navigate([`/inventory/transactions/gdn/addUpdatePurchaseReturns/${val.gdnId}`]);
  }
  ...
  showActivePopup() {
    this.commonService.$addPopupEvent.pipe(takeUntil(this.$destroy)).subscribe(popup => {
      if (popup && popup.redirectFromPage === '/inventory/transactions/gdn/purchaseReturnList') {
        this.showAddPopup = popup.isShowAddPopup;
        this.router.navigate(['/inventory/transactions/gdn/addUpdatePurchaseReturns/-1']);
        // open add popup
      } else {
