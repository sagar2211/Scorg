...
import { distinctUntilChanged, tap, switchMap, catchError, map, takeUntil, debounceTime } from 'rxjs/operators';
...
export class PurchaseReturnAddUpdateComponent implements OnInit, OnDestroy {
  ...
  //     if (popup && popup.redirectFromPage === `/inventory/transactions/gdn/addUpdatePurchaseReturns/${this.gdnId}`) {
  ...
  navigateToList(): void {
    this.router.navigate(['/inventory/transactions/gdn/purchaseReturnList']);
  }
