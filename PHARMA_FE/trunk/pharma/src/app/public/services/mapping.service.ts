...
export class MappingService {
  ...
  getDepartmentList(param): Observable<any> {
    const reqUrl = environment.baseUrlPharma + '/HISView/getDepartmentList';
  ...
  GetUserStoreMappingByUserId(userId): Observable<any> {
    if (this.storesArray.length) {
      return of(this.storesArray);
    }
    const reqUrl = `${environment.INVENTORY_BaseURL}/Mapping/GetUserStoreMappingByUserId?userId=${userId}`;
    return this.http.get(reqUrl).pipe(map((res: any) => {
      if (res.status_code === 200 && res.status_message === 'Success' && res.data) {
        if (res.data === null) {
          return [];
        } else {
          this.storesArray = res.data.storeMapping;
          return res.data.storeMapping;
        }
      } else {
        return [];
      }
  ...
  getDebtorList(param): Observable<any> {
