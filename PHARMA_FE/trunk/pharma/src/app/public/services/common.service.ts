...
export class CommonService {
  ...
  divideGstCalculations(gstCode, gstPercentage) {
    let igstPercent = 0;
    let cgstPercent = 0;
    let sgstPercent = 0;
    if (gstCode === 'GST') {
      const halfValue = (+gstPercentage / 2).toFixed(2);
      cgstPercent = +halfValue;
      sgstPercent = +halfValue;
    } else if (gstCode === 'IGST' || gstCode === 'UGST') {
      igstPercent = +gstPercentage;
    }
    return {igstPercent, cgstPercent, sgstPercent};
  }
