    ...
    this.dashboardService.getAppointmentTypeStats().subscribe(res => {
      if (res) {
        ...
        this.z = d3Scale.scaleOrdinal()
          .range(['#3F51B5', '#11cdef', '#0ea048', '#6b486b', '#a05d56', '#d0743c', '#ff8c00']);
  ...
  private drawMultiSeriesBars() {
    this.g.append('g')
      ...
      .on('mouseover', function (d) {
        ...
        if (d.key === 'previlege') {
          tooltip.select('#tooltipValue').text('Previlege : ' + d.value);
        } else if (d.key === 'normal') {
          tooltip.select('#tooltipValue').text('Normal : ' + d.value);
        } else if (d.key === 'free') {
          tooltip.select('#tooltipValue').text('Free : ' + d.value);
        }
