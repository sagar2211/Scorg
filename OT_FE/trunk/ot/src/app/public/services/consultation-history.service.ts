...
export class ConsultationHistoryService {
  ...
  generateInvestigationHistoryTree(historyData) {
    const treAry = [];
    _.map(historyData, (hsd, pi1) => {
      const treObj = {
        name: hsd.serviceTypeName,
        id: 'node_' + pi1,
        children: [],
        value: {
          type: 'node_1',
          data: hsd,
          serviceType: hsd.serviceTypeId,
        },
        isExpanded: pi1 === 0 ? true : false,
        isActive: false,
        isCollapsed: pi1 === 0 ? false : true,
        hasChildren: true,
      };
      _.map(hsd.dateData, (dateDt, pi2) => {
        const treObjCh1 = {
          name: moment(dateDt.date).format('DD-MM-YYYY'),
          id: 'node_' + pi1 + '_' + pi2,
          children: [],
          value: {
            type: 'node_2',
          ...
          isExpanded: pi2 === 0 ? true : false,
          isActive: false,
          isCollapsed: pi2 === 0 ? false : true,
          hasChildren: true,
        };
        _.map(dateDt.data, (ddt, pi3) => {
          _.map(ddt, dt => {
            const treObjCh2 = {
              name: dt.orderType,
              id: 'node_' + pi1 + '_' + pi2 + '_' + pi3,
              children: [],
              value: {
                type: 'node_3',
                data: dt,
              ...
              isExpanded: pi3 === 0 ? true : false,
              isActive: false,
              isCollapsed: pi3 === 0 ? false : true,
              hasChildren: true,
            };
            _.map(dt.serviceData, (st, chi) => {
              const treObjCh3 = {
                name: st.serviceName,
                id: 'node_' + pi1 + '_' + pi2 + '_' + pi3 + '_' + chi,
                children: [],
                value: {
                  type: 'node_4',
                  data: st,
                  serviceType: hsd.serviceTypeId,
                  hashId: 'node_' + pi1 + '_' + pi2 + '_' + pi3 + '_' + chi
                },
                isExpanded: chi === 0 ? true : false,
                isActive: false,
                isCollapsed: chi === 0 ? false : true,
                hasChildren: false,
              };
              treObjCh2.children.push(treObjCh3);
            });
