...
export class FaqSectionsService {
  ...
  getSectionById(id): Observable<any> {
    ...
    // };
    // const patientId = this._consultationService.getPatientObj('patientId');
    // const activeFormFaqList = this._consultationService.getConsultationFormDataByKey(patientId, 'SectionHistorySetting');
    // if (activeFormFaqList) {
    //   const faqSectionById = _.find(activeFormFaqList, (o) => {
    //     return o.template_id == id;
    //   });
    //   if (faqSectionById !== undefined) {
    //     return of(faqSectionById);
    //   }
    // }

    // CAll to API by form id.
    // const reqUrl = environment.EMR_BaseURL +  '/PageSetting/getSectionHistorySettingsBySectionId_2';
    // return this.http.post(reqUrl, params).pipe(
    //   map((res: any) => {
    //     const result = res.data.data;
    //     this._consultationService.setConsultationFormData(patientId, 'SectionHistorySetting', result, 'add');
    ...
    _.map(localStorySettings, (question) => {
      ...
      if (queAnswerRowExists.length > 1) {
        ...
        _.map(queAnswerRowExists, (v, k) => {
          if (k > 0) {
            question.answerModelArray.push(Object.assign(question.answerModelArray[0]));
          }
        });
      }
          ...
          if (answerGroup.ansControlKey != 'check_box') {
            ...
            if (answerExists.length > 0 && answerGroup.ansControlKey != 'multi_select') {
              if (answerGroup.ansControlKey == 'datepicker') {
                ...
                answerGroup.selectedAns = answerExists[0].answer_id;
              }
            } else if (answerExists.length > 0 && answerGroup.ansControlKey == 'multi_select') {
              _.map(answerExists, (ans) => {
                answerGroup.selectedAns.push({ id: ans.answer_id });
              });
            }
            ...
            _.map(answerGroup.optionsData, (option) => {
              ...
              option.selectedAns = '';
              if (!_.isUndefined(answerExists)) {
                option.selectedAns = answerExists.answer_id + '';
              }
            });
          }
        });
        ...
        _.map(answerRowData, (answerGroup) => {
          if (answerGroup.ansControlKey == 'check_box') {
            ...
            if (answerGroup.ansControlKey == 'input_box' || answerGroup.ansControlKey == 'text_area' || answerGroup.ansControlKey == 'datepicker') {
              if (answerGroup.ansControlKey == 'datepicker') {
                const selectedDate = moment(answerGroup.selectedAns).format('DD-MM-YYYY');
                answer = selectedDate;
              } else {
            ...
            const ansDataObject = {
              ...
              ans_group_key: answerGroup.ansGroup,
              control_key: answerGroup.ansControlKey,
              answer_id: answerGroup.selectedAns,
              answer_text: answer,
              story_setting: question.storySetting,
              form_id: question.categoryId,
              form_name: question.categoryName,
