...
export class MaterialIndentListComponent implements OnInit, OnDestroy {
  ...
  showActivePopup() {
    this.commonService.$addPopupEvent.pipe(takeUntil(this.$destroy)).subscribe(popup => {
      if (popup && popup.redirectFromPage === '/inventory/indent/materialIndentList') {
        this.showAddPopup = popup.isShowAddPopup;
        this.router.navigate(['/inventory/indent/addMaterialIndent']);
      } else {
  ...
  redirectUpdatePage(row) {
    this.router.navigate(['/inventory/indent/updateMaterialIndent/' + row.indentId]);
  }
