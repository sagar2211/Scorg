...
@Component({
  selector: 'app-issue-summery-list',
  templateUrl: './issue-summery-list.component.html',
  styleUrls: ['./issue-summery-list.component.scss']
})
export class IssueSummeryListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.statusListArray.push(Constants.issueStatusOpen);
    this.statusListArray.push(Constants.issueStatusPartiallyAccepted);
    this.statusListArray.push(Constants.issueStatusAccepted);
  }
  ...
  defaultObject() {
    ...
    this.sortingObject = { sort_order: 'desc', sort_column: 'issueNo' };
    this.searchString = '';
  ...
  getListData() {
    ...
    this.issueService.getIssueList(params).subscribe(res => {
      if (res.totalCount > 0) {
  ...
  showActivePopup() {
    this.commonService.$addPopupEvent.pipe(takeUntil(this.$destroy)).subscribe(popup => {
      if (popup && popup.redirectFromPage === '/inventory/issue/materialIndentList') {
        this.showAddPopup = popup.isShowAddPopup;
        // this.router.navigate(['/inventory/issue/addIssue']);
      } else {
  ...
  redirectUpdatePage(row) {
    this.router.navigate(['/inventory/issue/addUpdateIssue/' + row.issueId + '/issue']);
  }
  ...
  getPrintData(data) {
    const url = environment.REPORT_API + 'Report/IssuePrint/?auth_token=' + this.authService.getAuthToken() + '&issueId=' + data.issueId;
  ...
  showAuditLog(row): void {
    ...
    modalInstance.componentInstance.inputData = {
      stageId: Constants.auditLogStageConstants.Issue,
      referenceId: row.issueId
    };
    modalInstance.result.then((result) => {
    });
  }
