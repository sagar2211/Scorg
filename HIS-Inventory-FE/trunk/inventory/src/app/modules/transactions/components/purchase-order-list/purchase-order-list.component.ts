...
export class PurchaseOrderListComponent implements OnInit, OnDestroy {
  ...
  showActivePopup() {
    this.commonService.$addPopupEvent.pipe(takeUntil(this.$destroy)).subscribe(popup => {
      if (popup && popup.redirectFromPage === '/inventory/transactions/purchaseOrders') {
        this.showAddPopup = popup.isShowAddPopup;
        this.router.navigate(['/inventory/transactions/addPurchaseOrder']);
      } else {
  ...
  redirectUpdatePage(row) {
    this.router.navigate(['/inventory/transactions/editPurchaseOrder/' + row.id]);
  }
