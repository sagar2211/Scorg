    ...
    if (this.authService.isLoggedIn()) {
      if (this.authService.isUserLoggedIn()) {
        if (this.redirectUrl) { // -- go to previous attempt url
          ...
          const key = this.appkey === 'qms' ? 'QMS' : 'APPOINTMENT';
          const appData = _.find(userInfo.assigenedApplication, d => {
            return d.app_key === key;
          });
          const menuList = this.appkey === 'qms' ? this.sideMenuService.sideMenuListQms : this.sideMenuService.sideMenuListAppointment;
          if (this.appkey === 'qms') {
            if (appData && appData.primary_permission && _.find(userPermissions, (o) => o === appData.primary_permission.name)) {
              const check = this.sideMenuService.redirecToLanding(appData.primary_permission.name, menuList);
              if (check.link && check.redirect) {
                this.router.navigate(['/app/qms/' + check.link]);
              } else if (userInfo.role_type === ApplicationEntityConstants.DOCTOR
            ...
            if (appData && appData.primary_permission && _.find(userPermissions, (o) => o === appData.primary_permission.name)) {
              const check = this.sideMenuService.redirecToLanding(appData.primary_permission.name, menuList);
              if (check.link && check.redirect) {
                this.router.navigate(['/appointmentApp/' + check.link]);
                ...
                this.router.navigate(['/appointmentApp/appointments/search/searchAppointment']);
              } else if (userInfo.role_type === ApplicationEntityConstants.TELECALLER
                && _.find(userPermissions, (o) => o === PermissionsConstants.View_Quick_Book_Appointment)) {
