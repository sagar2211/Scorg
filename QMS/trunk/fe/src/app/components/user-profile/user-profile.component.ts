...
import { QSlotInfoDummy } from 'src/app/modules/qms/models/q-entity-appointment-details';
...
import { CallingConfirmLibComponent, QmsQlistLibService, QAppointmentDetails, QueueSlots } from '@qms/qlist-lib';
import { DefaultLandingSelectionComponent } from 'src/app/default-landing-selection/default-landing-selection.component';

@Component({
  selector: 'app-user-profile',
  templateUrl: './user-profile.component.html',
  styleUrls: ['./user-profile.component.scss'],
  providers: [DisplayDataByStatusPipe, QmsQlistLibService]
})
export class UserProfileComponent implements OnInit, OnDestroy {
  ...
  timeFormateKey = '';
  pendingActionCallingLimit = 0;
  repeatCallingCount = 0;
  userInfo = null;
  ...
  ngOnInit() {
    ...
    this.commonService.getQueueSettings(Constants.queueSkipSettingKey, this.userId).subscribe(res => {
      this.pendingActionCallingLimit = +res.no_of_pending_action;
      this.repeatCallingCount = +res.no_of_repeat_calling;
    });
    // const logoutsub = this.commonService.$getlogOutEvent.pipe(takeUntil(this.$destroy)).subscribe((res: any) => {
  ...
  //   if (changes.isLogOutFlag.currentValue) {
  //     this.logout();
  //   }
  // }
  ...
  getTitleList(): Observable<any> {
    return this.userService.getTitles().pipe(map(res => {
  ...
  showProfileDetails(userDetails) {
    if (userDetails != null && !_.isUndefined(userDetails)) {
      ...
      key = key === 'appointment' ? 'APPOINTMENT' : 'QMS';
      this.userObjDetail = {
        id: userDetails.id,
  ...
  updateLandingPage() {
    ...
    const modalInstance = this.changePasswordModalService.open(DefaultLandingSelectionComponent,
      {
  ...
  logout(): void {
    if (!this.isFrontDeskUser) {
      ...
      const getCheckInCheckOutFork = this.getCheckInCheckOutStatusByUserId();
      // const getQueueAppointmentSlotsFork = this.getQueueAppointmentSlots();
      forkJoin([getCheckInCheckOutFork]).subscribe((res: any) => {
        ...
        if (res[0]) {
          ...
          // this.getQueueAppointmentSlots().subscribe(res1 => {
          //   const appList = this.displayDataByStatusPipe.transform(this.qSlotList, true, ['INCONSULTATION', 'CALLING', 'SKIP'], true);
          //   if (appList) {
          ...
          const dt = this.providerDetails.find(p => (p.providerId === currentProviderDetails.providerId) && (p.providerValueId === currentProviderDetails.providerValueId));
          if (dt) {
            this.checkProvidersAppointments(dt, true);
          }
