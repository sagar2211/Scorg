...
export class AppComponent implements OnDestroy, OnInit {
  ...
  ngOnInit() {
    if (!this.authService.getActiveAppKey()) {
      this.authService.setActiveAppKey('qms');
      localStorage.setItem('loginFor', JSON.stringify('qms'));
    } else if(this.authService.getActiveAppKey() === 'appointment') {
      this.router.navigate(['/appointmentApp/appointments/']);
    }
    setTimeout(() => {
      const key = this.authService.getActiveAppKey();
      const appKey = _.toLower(key) === 'qms' ? 'QMS' : 'APPOINTMENT';
